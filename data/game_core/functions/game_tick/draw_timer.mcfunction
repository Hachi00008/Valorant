...
# ボスバーに表示
    execute if score $Sec Timer matches ..9 run bossbar set valorant:timer name ["",{"text":"\uE002","font":"ui"},{"translate":"space.-32","font":"space"},{"translate":"space.-16","font":"space"},{"score": {"name": "$Min","objective": "Timer"},"font": "up"},{"text": ":0","font": "up"},{"score": {"name": "$Sec","objective": "Timer"},"font": "up"}]
